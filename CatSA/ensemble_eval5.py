...
def main() -> None:
    ...
    print(f"[phase5] Load 4 CatSA cũ + bản A + 2 CORE trên {device} ...", flush=True)
    ...
    variants = _variants()
    print(f"[phase5] {len(variants)} biến thể — VAL ...", flush=True)
    val_res, n_val = _eval(
    ...
    if best in test_res:
        m = test_res[best]
        print(f"\n[phase5] Combo chọn theo VAL: {best}")
        print(f"[phase5] → TEST chính thức: mrr@20={m['mrr@20']:.4f} | hr@20={m['hr@20']:.4f} | ndcg@20={m['ndcg@20']:.4f}")

    with open("checkpoints/ensemble_eval5_result.yaml", "w", encoding="utf-8") as f:
        yaml.safe_dump({
            "val_top12": {c: v for c, v in ranked[:12]},
            "test": test_res,
            "combo_chon_theo_val": best,
        }, f, allow_unicode=True, sort_keys=False)
    print("[phase5] Đã ghi checkpoints/ensemble_eval5_result.yaml")
