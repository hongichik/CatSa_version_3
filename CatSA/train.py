...
def _model_scores(model, z_s: torch.Tensor, batch=None) -> torch.Tensor:
    """Gọi scores(); truyền batch nếu encoder hỗ trợ (mg_core extra rerank)."""
    import inspect
...
def train_model(data: dict, cfg: Config):
    ...
    augmenter = CatSAAugmenter(
        item2cat, data["cat2items"], cat_parent, cfg.augment, unk_cat_idx=unk_cat_idx,
    ...
    for epoch in range(1, tr.max_epochs + 1):
        ...
        log.info(
            "Epoch %d: total=%.4f, rec=%.4f, cl=%.4f, aux=%.4f (%.1fs)",
            epoch, total_loss / n_seen, total_rec / n_seen,
            total_cl / n_seen, total_aux / n_seen, time.time() - t0,
        )
