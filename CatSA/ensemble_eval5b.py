"""Phase 5 — ensemble 7 model: 4 CatSA cũ + bản A + CORE seed42 + CORE seed43.

Phase 4 (6 model) = test 0.3896 (old0.4|A0.3|core0.3|rep10) — thiếu 0.0004
tới mục tiêu 0.39. Thêm CORE seed43 (test đơn 0.3715) tăng đa dạng.
Grid quanh vùng tốt nhất phase 4; phần trọng số core chia đều seed42/43.

Chạy:
    python CatSA/ensemble_eval5.py
"""
    ...
    for wo in W_OLDS:
        for wa in W_AS:
            ...
            if wc <= 0:
                continue
            for dr in D_REPS:
                # (w_old, w_A, w_core42, w_core43, rep): chia đều 2 CORE +
                # thêm biến thể chỉ dùng core42 để đối chứng
                out.append((wo, wa, wc / 2, wc / 2, dr))
                out.append((wo, wa, wc, 0.0, dr))
    return out


@torch.no_grad()
def _eval(loader, old_models, model_a, core_models, variants, n_items, device):
    keys = [
        f"old{wo:g}|A{wa:g}|c42_{w42:g}|c43_{w43:g}|rep{dr:g}"
        for wo, wa, w42, w43, dr in variants
    ]
    ...
    for batch, _, targets in loader:
        ...
        core_lgs = [_core_logits(cm, sessions, device) for cm in core_models]

        rep_flat = torch.zeros(B, n_items, device=device)
        for i, s in enumerate(sessions):
            rep_flat[i, torch.tensor(list(set(s)), device=device)] = 1.0

        for (wo, wa, w42, w43, dr), c in zip(variants, keys):
            fused = (
                wo * old_logits + wa * a_logits
                + w42 * core_lgs[0] + w43 * core_lgs[1] + dr * rep_flat
            )
...
def main() -> None:
    ...
    print(f"[phase5b] Load 4 CatSA cũ + bản A + 2 CORE trên {device} ...", flush=True)
    old_models = [_load_catsa_model(d, data, device) for _, d in CATSA_CKPTS]
    model_a = _load_catsa_model(CKPT_A, data, device)
    core_models = [_load_core_model(d, n_items, device) for d in CORE_DIRS]

    variants = _variants()
    print(f"[phase5b] {len(variants)} biến thể — VAL ...", flush=True)
    val_res, n_val = _eval(
        loader_for(data["val_sessions"]), old_models, model_a, core_models, variants, n_items, device,
    )
    ranked = sorted(val_res.items(), key=lambda kv: kv[1]["mrr@20"], reverse=True)
    print(f"\n[phase5b] VAL ({n_val} mẫu) — top 12:")
    for c, m in ranked[:12]:
        print(f"  {m['mrr@20']:.4f}  hr@20={m['hr@20']:.4f}  {c}")

    key2var = dict(zip(
        [f"old{wo:g}|A{wa:g}|c42_{w42:g}|c43_{w43:g}|rep{dr:g}" for wo, wa, w42, w43, dr in variants],
        variants,
    ))
    top_vars = [key2var[c] for c, _ in ranked[:6]]
    print("\n[phase5b] TEST với top-6 theo val ...", flush=True)
    test_res, n_test = _eval(
        loader_for(data["test_sessions"]), old_models, model_a, core_models, top_vars, n_items, device,
    )
    print(f"\n[phase5b] KẾT QUẢ TEST ({n_test} mẫu):")
    ...
    if best in test_res:
        m = test_res[best]
        print(f"\n[phase5b] Combo chọn theo VAL: {best}")
        print(f"[phase5b] → TEST chính thức: mrr@20={m['mrr@20']:.4f} | hr@20={m['hr@20']:.4f} | ndcg@20={m['ndcg@20']:.4f}")

    with open("checkpoints/ensemble_eval5b_result.yaml", "w", encoding="utf-8") as f:
        yaml.safe_dump({
            "val_top12": {c: v for c, v in ranked[:12]},
            "test": test_res,
            "combo_chon_theo_val": best,
        }, f, allow_unicode=True, sort_keys=False)
    print("[phase5b] Đã ghi checkpoints/ensemble_eval5b_result.yaml")
