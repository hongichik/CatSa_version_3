...
def main() -> None:
    ...
    print(f"[phase6] len_gate_seed43 sẵn sàng: {use_lg43}", flush=True)
    print(f"[phase6] Load models trên {device} ...", flush=True)
    ...
    print(f"[phase6] {len(variants)} biến thể × 3 bucket — VAL ...", flush=True)
    val_res, n_val = _eval_per_bucket(
        loader_for(data["val_sessions"]), old_models, model_a, rest_models, variants, n_items, device,
    )

    bucket_weights = {}
    print("\n[phase6] Chọn trọng số riêng từng bucket (theo val mrr@20):")
    ...
    print("\n[phase6] TEST với bộ trọng số ghép theo bucket ...", flush=True)
    ...
    print(f"\n[phase6] KẾT QUẢ TEST ({n_test} mẫu):")
    print("  " + " | ".join(f"{k}={v:.4f}" for k, v in test_metrics.items()))
    print(f"\n[phase6] → TEST chính thức: mrr@20={test_metrics['mrr@20']:.4f}")

    with open("checkpoints/ensemble_eval6_result.yaml", "w", encoding="utf-8") as f:
        yaml.safe_dump({
            "use_lengate_seed43": use_lg43,
            "bucket_weights": {lb: list(w) for lb, w in bucket_weights.items()},
            "test_metrics": test_metrics,
        }, f, allow_unicode=True, sort_keys=False)
    print("[phase6] Đã ghi checkpoints/ensemble_eval6_result.yaml")
