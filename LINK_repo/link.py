...
class LINK(SLIST):
    ...
    def fit(self, config, dataset):
        ...
        # SLIS matrix (~n_items² float32) no longer needed after LIS features are built
        self.slis_matrix = None
        gc.collect()
        ...
        XtW = input_matrix.transpose().dot(W2)
        XtWS = XtW.dot(target_matrix).toarray()
        G = XtW.dot(input_matrix).toarray()
        del XtW, input_matrix, target_matrix, input1, input2, target1, target2, W2, w2, row_weight1, row_weight2
        gc.collect()
        G_time = time() - G_start
        self.logger.info(f"[{G_time:.2f}s] G is made. Sparsity:{(1 - np.count_nonzero(G)/(self.n_items**2))*100}%")

        rhs = XtWS + self.reg_teacher * teacher_matrix
        del XtWS, teacher_matrix
        gc.collect()
        ...
        del G
        gc.collect()
        A.ravel()[:: A.shape[0] + 1] += self.reg_teacher
        lu, piv = lu_factor(A, overwrite_a=False, check_finite=False)
        del A
        self.enc_w = lu_solve((lu, piv), rhs, check_finite=False)
        self.enc_w = np.nan_to_num(self.enc_w, copy=False)
        del rhs, lu, piv
        gc.collect()
        P_time = time() - P_start
        self.logger.info(f"[{P_time:.2f}s] closed-form solve done")
    ...
    def make_SLIST(self, dataset, method):
        ...
        if self.save_load_SLIST and os.path.exists(SLIST_path):
            print(f"Load {method} matrix (mmap)")
            return np.load(SLIST_path, mmap_mode="r")
        else:
        ...
        XtWS = XtW.dot(target_matrix).toarray()
        G = XtW.dot(input_matrix).toarray()
        del XtW, input_matrix, target_matrix, W2
        gc.collect()
        G_time = time() - G_start
        ...
        del G
        gc.collect()
        A.ravel()[:: A.shape[0] + 1] += self.reg
        lu, piv = lu_factor(A, overwrite_a=False, check_finite=False)
        del A
        enc_w = lu_solve((lu, piv), XtWS, check_finite=False)
        enc_w = np.nan_to_num(enc_w, copy=False)
        del XtWS, lu, piv
        gc.collect()
        P_time = time() - P_start
        print(f"[{P_time:.2f}s] closed-form solve done (reg:{self.reg})")
        print(f"[{time()-train_start:.2f}s] training is done")

        np.save(SLIST_path, enc_w)
        del enc_w
        gc.collect()
        print(f"Save {method} matrix → mmap {SLIST_path}")
        return np.load(SLIST_path, mmap_mode="r")
